    ...
    ERC1155TokenReceiver
{
    ...
    modifier onlyValidParameters(OrderParameters[] calldata parameters) {
        for (uint256 i; i < parameters.length; ) {
            if (
                orders[parameters[i].idHash()] != parameters[i].hash() ||
                owners[parameters[i].idHash()] != msg.sender
            ) {
                revert InvalidOrder();
            }
    ...
    function createOrder(OrderParameters[] memory parameters)
        external
        whenNotPaused
    {
        for (uint256 i; i < parameters.length; ) {
    ...
    function updateOrder(
        OrderParameters[] calldata existingParameters,
        OrderParameters[] calldata newParameters
    ) external whenNotPaused onlyValidParameters(existingParameters) {
    ...
    function withdraw(OrderParameters[] calldata parameters)
        external
    ...
    function createOrderForReceivedItems(OrderParameters[] calldata parameters)
        external
    {
        for (uint256 i; i < parameters.length; ) {
    ...
    function setOwnersForItems(
        TokenIdentifier[] calldata tokens,
        address[] calldata ownerArray
    ) external onlyOwner {
    ...
    function onERC721Received(
        ...
        if (collection.supportsInterface(type(IERC721).interfaceId)) {
            owners[
                TokenIdentifier({collection: address(collection), id: tokenId})
                    .hash()
            ] = from;
            delete orders[
                TokenIdentifier({collection: address(collection), id: tokenId})
    ...
    function withdrawERC721(
        ...
        for (uint256 i; i < ids.length; ) {
            delete orders[
                TokenIdentifier({collection: address(token), id: ids[i]}).hash()
            ];
            delete owners[
                TokenIdentifier({collection: address(token), id: ids[i]}).hash()
            ];
